...
| [Task-class enum (§2a candidate assignment)](2026-08-14-task-class-enum.md) | **FILED 2026-08-14, awaiting operator ratification (charter row 6)** — the crosswalk-§2a sibling publication: 200 observed S3 values → 13 classes + reserved `other`; scope and review-generation axes preserved per panel A12; stated losses recoverable from `class_free`. Checker: `check_task_class_enum.py` (repo root, fail-loud on vocabulary growth). | Operator ratifies the assignment + the `other`-member sub-decision; ratification enacts the crosswalk §2a cross-ref + version bump in the same pass. Enforcement of `class` as REQ stays a separate later amendment. |
| [S3 projector intent-side rules](2026-08-14-s3-projector-intent-rules.md) | **FILED 2026-08-14, awaiting operator ratification (charter row 6)** — the one unspecified intent REQ field (`surface`: projected records omit, read as unrecorded) + three readings to confirm (harness_contract v2-only, loss-stated coercions, last-terminal-wins). Implemented dark in delegate-runtime `e5226ed`; attestation floor keeps all projected evidence non-decision-grade meanwhile. | Operator ratifies → §2 surface-row qualifier + version bump + seed-alias extension. |
